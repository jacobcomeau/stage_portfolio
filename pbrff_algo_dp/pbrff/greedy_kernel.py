...
class GreedyKernelLearner(object):
    ...
    def DPRFF(self, Dmax):
        n = self.loss.size
        V = np.zeros(shape=(n + 1, Dmax + 1))
        rff_index = self.random_state.choice(self.omega.shape[1], Dmax, replace=True, p=self.pb_Q)
        W = np.full((n,), 2, dtype=int)
        for index in rff_index:
            W[index] = 1

        for i in range(1, n + 1):
            for j in range(1, Dmax + 1):
                if j - W[i - 1] >= 0:
                    V[i][j] = max(V[i - 1][j], (1000 / self.loss[i - 1]) + V[i - 1][j - W[i - 1]])
                else:
                    V[i][j] = V[i - 1][j]

        return W, V
    
    def OptimalRFF(self, V, W, Dmax):
        L = []
        n = self.loss.size
        j = Dmax
    ...
    def bayesian_optimization_(self, trial, D, method, results):
        ...
        if method == "base":
            ...
            nbrD_choisi = D
            kernel_features = self.omega[:, results]
        ...
        score = 1 - accuracy_score(self.dataset['y_valid'], clf.predict(transformed_X_valid))
    ...
    def bayesian_optimiation(self, D, method, results):
        optuna.logging.set_verbosity(optuna.logging.WARNING) # Équivalent à verbose = 0
        study = optuna.create_study()
        f = partial(self.bayesian_optimization_, method=method, D=D, results=results)
    ...
    def learn_pbrff(self, D, method, C, results):
        ...
        print(f"Model A : Train Score -> {train_err} ; Val Score -> {err} ; Test Score -> {test_err}")
        print(f"Nbr de RFF choisi : {len(results)}")
        ...
        if method == "base":
            ...
            # Pour le modèle avec l'algorithme vorace
            return dict([("dataset", self.dataset['name']), ("exp", 'greedy'), ("algo", 'PBRFF'), ("C", C), ("D", D), ("N", self.N), \
                    ("gamma", self.gamma), ("beta", self.beta), ("nbrD_choisi", nbrD_choisi), ("train_error", train_err), ("val_error", val_err), \
                    ("test_error", test_err), ("f1", f1), ("time", self.time)])
...
def compute_greedy_kernel(args, greedy_kernel_learner_file, gamma, D_range, random_state):
    ...
    if args["algo"] == "rff":
        ...
        for D in D_range:
            ...
            W, V = greedy_kernel_learner.DPRFF(D)
            results = greedy_kernel_learner.OptimalRFF(V, W, D)
            best_params = greedy_kernel_learner.bayesian_optimiation(D, "greedy", results)

            #tmp_results.append(greedy_kernel_learner.learn_pbrff(D, "base", best_params))
            #tmp_results.append(greedy_kernel_learner.learn_pbrff(D, "greedy", best_params[0], best_params[1], best_params[2], best_params[3]))
            tmp_results.append(greedy_kernel_learner.learn_pbrff(D, "greedy", best_params, results))
