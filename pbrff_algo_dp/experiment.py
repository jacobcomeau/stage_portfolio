...
def main():
    ...
    hps = {'gamma': np.logspace(-7, 2, 10),
           ...
           'greedy_kernel_D': np.array([5000], dtype=int),
           'maxTry' : [2, 4, 5, 7, 10],
           'p' : [1, 5, 10, 15, 20],
    ...
    if "greedy_kernel" in args.experiments:
        ...
        param_grid = ParameterGrid([{'algo': ["pbrff"], 'param': hps['beta']}]) # TODO Fonction avant
        #param_grid = ParameterGrid([{'algo': ["pbrff"], 'param': np.array([args.beta])}])
        ...
        results_files = {join(paths['greedy_kernel'], f"{p['algo']}_jacob_test" + (f"_{p['param']}.pkl" if ('param' in p) else ".pkl")): p \
                                                                                                            for p in param_grid}
        #results_to_compute = [dict({"output_file":f}, **p) for f, p in results_files.items() if not(exists(f))]
        results_to_compute = [dict({"output_file":f}, **p) for f, p in results_files.items()]
        ...
        if results_to_compute:
            ...
            for element in results_to_compute:
                ...
                #compute_greedy_kernel(element, greedy_kernel_learner_cache_file, gamma, np.array([args.rff]), random_state)
            # computed_results = list(Pool(processes=n_cpu).imap(parallel_func, results_to_compute)) # TODO Fonction avant
